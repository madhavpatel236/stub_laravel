    <div>
        <form method='post' action='{{ route('Test03Controller.store') }}' name='data_input_form'>
            @csrf

            <lable> test02:</lable>
            <input class= "integer" , name="test02" /> <br /> <br />
    ...
    <script>
        $(document).ready(function() {
            $.ajax({
                url: "{{ route('Test03Controller.index') }}",
                type: "get",
                data: {},
                success: function(res) {
                    ...
                    res.forEach(row => {
                        ...
                        rows += "<td class=row["id"] id=row["id"]" > < button > Edit < /button></td > < /tr> ;
                    });
