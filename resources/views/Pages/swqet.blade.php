    ...
    <script>
            ...
            $.ajax({
                url: '{{ route('SwqetController.store') }}',
                method: 'POST',
                data: formData,
        ...
        $(document).on('click', '.edit-btn', function() {
            ...
            $('#data-id').val();
            let editteUrl = '{{ route('SwqetController.edit', ['SwqetController' => 'id']) }}'.replace(
                'id', userId);
        ...
        $(document).on('click', '.delete-btn', function() {
            ...
            let deleteUrl = '{{ route('SwqetController.destroy', ['SwqetController' => 'id']) }}'.replace(
                'id', userId);
        ...
        $('#data_update_btn').on('click', function() {
            ...
            let updateUrl = '{{ route('SwqetController.update', ['SwqetController' => ':id']) }}'.replace(
                ':id', userId);
    ...
    function fetchData() {
        $.ajax({
            url: '{{ route('SwqetController.index') }}',
            type: 'GET',
...
if(a == "" || a == null){
 ...
 if(typeof a == 'string' == 'string' && a.length() > 21){
                ...
                var b = $('#b').val().trim();
if(b == "" || b == null){
                $('#b_error').val('b is required!!')
                }
 if(typeof b == 'string' == 'integer' && b.length() > ){
                $('#b_error').val(' Max allowed field length is: ')
            }
                var c = $('#c').val().trim();
if(c == "" || c == null){
                $('#c_error').val('c is required!!')
                }
 if(typeof c == 'string' == 'text' && c.length() > ){
                $('#c_error').val(' Max allowed field length is: ')
            }
    }
