    <div>
        <form method='post' id='data_input_form' name='data_input_form'>
            ...
            <lable> G:</lable>
            <input class= "integer" , name="G" /> <br /> <br />

            <lable> GG:</lable>
            <input class= "integer" , name="GG" /> <br /> <br />

            <lable> GGG:</lable>
            <input class= "integer" , name="GGG" /> <br /> <br />
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                $('#data-id').val();
                let editteUrl = '{{ route('GGController.edit', ['GGController' => 'id']) }}'.replace(
                    'id', userId);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('input[name='
                            Name ']'
                            ').val(data.Name);
                            $('#edit_id').val(userId); $('#data_update_btn').show(); $(
            ...
            $(document).on('click', '.delete-btn', function() {
                ...
                let deleteUrl = '{{ route('GGController.destroy', ['GGController' => 'id']) }}'.replace(
                    'id', userId);
            ...
            $('#data_update_btn').on('click', function() {
                ...
                // let updateUrl = '{{ route('GGController.update', ['GGController' => ':id']) }} '
                //     .replace(':id', userId)
                // ';

                let updateUrl = "{{ route('GGController.update', ['GGController' => ':id']) }}".replace(
                    ':id', userId);
