    ...
    <script>
        $(document).ready(function() {
            ...
            $('#data_submit_btn').on('click', function() {
                ...
                $.ajax({
                    url: '{{ route('SwqettyController.store') }}',
                    method: 'POST',
                    data: formData,
            ...
            $(document).on('click', '.edit-btn', function() {
                    ...
                    $('#data-id').val();
                    let editteUrl = '{{ route('SwqettyController.edit', ['SwqettyController' => 'id']) }}'
                        .replace(
                            'id', userId);
        ...
        $(document).on('click', '.delete-btn', function() {
            ...
            let deleteUrl = '{{ route('SwqettyController.destroy', ['SwqettyController' => 'id']) }}'.replace(
                'id', userId);
        ...
        $('#data_update_btn').on('click', function() {
            ...
            let updateUrl = '{{ route('SwqettyController.update', ['SwqettyController' => ':id']) }}'.replace(
                ':id', userId);
        ...
        function fetchData() {
            $.ajax({
                url: '{{ route('SwqettyController.index') }}',
                type: 'GET',
        ...
        $(document).ready(function() {
            ...
            $('#data_input_form').on('submit', function(e) {
                ...
                if (typeof a == 'string' == 'string' && a.length() > 21) {
                    $('#a_error').html(' Max allowed field length is: 21')
                    flag = false;
                }
