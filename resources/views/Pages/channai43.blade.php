    ...
    @csrf
                ...
                <lable> age:</lable>
                <input class= "binary" id="age" name="age"/>
                <span class= "binary_error" id="age_error" name="age_error"></span><br/> <br/>
    ...
    <script>
            ...
            $.ajax({
                url: '{{ route('Channai43Controller.store') }}',
                method: 'POST',
        ...
        $(document).on('click', '.edit-btn', function() {
            ...
            $('#data-id').val();
            let editteUrl = '{{ route('Channai43Controller.edit', ['Channai43Controller' => 'id']) }}'.replace(
                'id', userId);
        ...
        $(document).on('click', '.delete-btn', function() {
            ...
            let deleteUrl = '{{ route('Channai43Controller.destroy', ['Channai43Controller' => 'id']) }}'.replace(
                'id', userId);
        ...
        $('#data_update_btn').on('click', function(e) {
            ...
            let updateUrl = '{{ route('Channai43Controller.update', ['Channai43Controller' => ':id']) }}'.replace(
                ':id', userId);
    ...
    function fetchData() {
        $.ajax({
            url: '{{ route('Channai43Controller.index') }}',
            type: 'GET',
...
if(age == "" || age == null){
 ...
 if(age.length >  255){
                $('#age_error').html(' Max allowed field length is: 255')
            flag = false;
                }
if (flag != true) {
                    e.preventDefault();
