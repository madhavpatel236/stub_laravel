    <div>
        <form method='post' id='data_input_form' name='data_input_form'>
            ...
            <lable> a:</lable>
            <input class= "string" id="a" name="a" />
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#data_submit_btn').on('click', function() {
                ...
                $.ajax({
                    url: '{{ route('Punjab6Controller.store') }}',
                    method: 'POST',
                    data: formData,
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                $('#data-id').val();
                let editteUrl = '{{ route('Punjab6Controller.edit', ['Punjab6Controller' => 'id']) }}'
                    .replace(
                        'id', userId);
            ...
            $(document).on('click', '.delete-btn', function() {
                ...
                let deleteUrl = '{{ route('Punjab6Controller.destroy', ['Punjab6Controller' => 'id']) }}'
                    .replace(
                        'id', userId);
            ...
            $('#data_update_btn').on('click', function() {
                ...
                let updateUrl = '{{ route('Punjab6Controller.update', ['Punjab6Controller' => ':id']) }}'
                    .replace(
                        ':id', userId);
        ...
        function fetchData() {
            $.ajax({
                url: '{{ route('Punjab6Controller.index') }}',
                type: 'GET',
                success: function(res) {
                    ...
                    data.forEach(function(row) {
                        ...
                        rows += '<td>' +
                            '<button class='
                        edit - btn ' data-id=' + row.id + '>Edit</button> ' +
                            '<button class='
