    ...
    @csrf
                ...
                <lable> dc:</lable>
                <input class= "integer" id="dc" name="dc"/>
                <span class= "integer_error" id="dc_error" name="dc_error"></span><br/> <br/>
    ...
    <script>
        ...
        $(document).on('click', '.edit-btn', function() {
            ...
            $('#data-id').val();
            let editteUrl = '{{ route('Dc3Controller.edit', ['Dc3Controller' => 'id']) }}'.replace(
                'id', userId);
        ...
        $(document).on('click', '.delete-btn', function() {
            ...
            let deleteUrl = '{{ route('Dc3Controller.destroy', ['Dc3Controller' => 'id']) }}'.replace(
                'id', userId);
        ...
        $('#data_update_btn').on('click', function() {
            ...
            let updateUrl = '{{ route('Dc3Controller.update', ['Dc3Controller' => ':id']) }}'.replace(
                ':id', userId);
...
var flag = true;
                ...
                var dc = $('#dc').val().trim();
if(dc == "" || dc == null){
                $('#dc_error').html('dc is required!!')
                flag = false;
                }
 if(dc.length() >  255){
                $('#dc_error').html(' Max allowed field length is: 255')
            flag = false;
                }
if(dc != ^[0-9]){
                    $('#dc_error').html('Only numbers is allowed ')
                    flag = false;
                }
if (flag != true) {
                    e.preventDefault();
