    ...
    @csrf
                ...
                <lable> age:</lable>
                <input class= "string" id="age" name="age"/>
                <span class= "string_error" id="age_error" name="age_error"></span><br/> <br/>
    ...
    <script>
            ...
            $.ajax({
                url: '{{ route('Channai11Controller.store') }}',
                method: 'POST',
                data: formData,
        ...
        $(document).on('click', '.edit-btn', function() {
            ...
            $('#data-id').val();
            let editteUrl = '{{ route('Channai11Controller.edit', ['Channai11Controller' => 'id']) }}'.replace(
                'id', userId);
        ...
        $(document).on('click', '.delete-btn', function() {
            ...
            let deleteUrl = '{{ route('Channai11Controller.destroy', ['Channai11Controller' => 'id']) }}'.replace(
                'id', userId);
        ...
        $('#data_update_btn').on('click', function() {
            ...
            let updateUrl = '{{ route('Channai11Controller.update', ['Channai11Controller' => ':id']) }}'.replace(
                ':id', userId);
    ...
    function fetchData() {
        $.ajax({
            url: '{{ route('Channai11Controller.index') }}',
            type: 'GET',
...
if(age == "" || age == null){
 ...
 if(age.length() >  30){
                $('#age_error').html(' Max allowed field length is: 30')
            flag = false;
                }
if(age != ^[a-zA-Z]*$){
                    $('#age_error').html('Only numbers is allowed ')
                    flag = false;
                }
if (flag != true) {
                    e.preventDefault();
